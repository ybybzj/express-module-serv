...
function makeResolverFns(pSettings, options) {
  ...
  function resolveDepPath(dname) {
    ...
    result = normalizeFilepath(dpath, defaultExtensions, isResolveRecursive);
...
function normalizeFilepath(fpath, defaultExtensions, isResolveRecursive) {
  ...
  if(!isDir(fpath)){
    return _resolveFilePathWithoutExt(fpath, defaultExtensions);
  }

  var p_index = _resolveFilePathWithoutExt(fpath + '/index', defaultExtensions);
...
// }
var CMDKeywordsReg = /\b(?:require\s*\(|module\.exports\b|exports\.)/;
...
function generateStream(makeDepsStream, mappings, options, resolveDepPath){
  ...
  var excludModules = mapFlatten(modNamesFromStr(mappings.l || ''), resolveDepPath);
...
function _flatten(arr){
  return !Array.isArray(arr) ? [arr] : arr.reduce(function(m, item){
