...
function makeResolverFns(pSettings, options) {
  ...
  function resolveDepPath(dname) {
    ...
    if (dname in pSettings.files) {
      ...
      parts = _matchPathPrefix(dname, pSettings.files)
      if (parts) {
        ...
        parts = _matchPathPrefix(dname, paths) || dname.split('/');
...
function _matchPathPrefix(path, settings) {
  ...
  if(prefix) {
    return [prefix].concat(path.slice(prefix.length).split('/').filter(Boolean))
  }else{
    return null
  }
}
